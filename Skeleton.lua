...
local function DrawESPHook(EspBuilder)
    ...
    if localPlayer and localPlayer:IsAlive() then
        ...
        local player = EspBuilder:GetEntity()
        if player ~= nil and player:IsAlive() and player:GetTeamNumber() ~= localPlayerTeamNumber then
